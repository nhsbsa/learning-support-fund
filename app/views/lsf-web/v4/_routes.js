...
router.post('/v4/returnordecline', (req, res) => {
    ...
    if (returnstudent === 'Return to univerity') {
        res.redirect('/lsf-web/v4/TDAE-send-to-student')
    } else {
        res.redirect('/lsf-web/v4/TDAE-details-for-return')
    }

})

router.post('/v4/current-search-student', (req, res) => {

    const studentNumber = req.session.data['student-ref']
    req.session.data['studentRef'] = studentNumber

    res.redirect('/lsf-web/v4/current-search-student-results')


})

router.post('/v4/TDAE-accommodation-check', (req, res) => {
...
router.post('/v4/TDAE-authoriser-approved', (req, res) => {
...
router.post('/v4/TDAE-approve-pend-reject', (req, res) => {
    ...
    if (claimOutcome) {
        ...
        res.redirect('/lsf-web/v4/TDAE-provide-nhsbsa-comments')
    } else {
        res.redirect('/lsf-web/v4/TDAE-approve-pend-reject')
    }
...
router.post('/v4/TDAE-provide-nhsbsa-comments', (req, res) => {
    ...
    if (sendCommentsToNHSBSA === 'yes') {
        res.redirect('/lsf-web/v4/TDAE-nhsbsa-comments')
    } else {
        res.redirect('/lsf-web/v4/TDAE-claim-declaration')
    }
...
router.post('/v4/TDAE-claim-declaration', (req, res) => {
    ...
    if (declarationSigned) {
        ...
        res.redirect('/lsf-web/v4/TDAE-claim-submitted')
    } else {
        res.redirect('/lsf-web/v4/TDAE-claim-declaration')
    }
...
router.post('/v4/TDAE-details-for-return', (req, res) => {
    ...
    res.redirect('/lsf-web/v4/TDAE-return-cya')
